...
namespace wServer.realm.entities.player
{
    ...
    public partial class Player : Character, IContainer, IPlayer
    {
        ...
        protected override void ExportStats(IDictionary<StatsType, object> stats)
        {
            ...
            if (Boost != null)
            {
                //With Modifiers from Legendaries
                stats[StatsType.MaximumHP] = Stats[0] + Boost[0];
                stats[StatsType.MaximumMP] = Stats[1] + Boost[1];
                stats[StatsType.Attack] = Stats[2] + Boost[2] + AegisDamage() + MindStat2();
                stats[StatsType.Defense] = Stats[3] + Boost[3] + MindStat2() + SpikeDefense();
                stats[StatsType.Speed] = Stats[4] + Boost[4] + MindStat2() + AnguishNumber();
                stats[StatsType.Vitality] = Stats[5] + Boost[5] + MindStat2();
                stats[StatsType.Wisdom] = Stats[6] + Boost[6] + MindStat2();
                stats[StatsType.Dexterity] = Stats[7] + Boost[7] + MindStat2();
        ...
        {
            if (CheckDResistance() && HP <= MaxHp/4)
            {
                return 22;
            }
            else
        ...
        private bool CheckResistance()
        {
            ...
            {
                var item = Inventory[i];
                if (item == null || !item.Resistance) continue;
                return true;
            }
            return false;
        }
        private bool CheckCResistance()
        {
            ...
            {
                var item = Inventory[i];
                if (item == null || !item.CResistance) continue;
                return true;
            }
            return false;
        }
        private bool CheckPResistance()
        {
            ...
            {
                var item = Inventory[i];
                if (item == null || !item.PResistance) continue;
                return true;
            }
            return false;
        }
        public bool CheckDResistance()
        {
            ...
            {
                var item = Inventory[i];
                if (item == null || !item.DResistance) continue;
                return true;
            }
            return false;
        }
        public bool CheckMResistance()
        {
            ...
            {
                var item = Inventory[i];
                if (item == null || !item.MResistance) continue;
                return true;
            }
            return false;
        }
        public bool CheckAResistance()
        {
            ...
            {
                var item = Inventory[i];
                if (item == null || !item.AResistance) continue;
                return true;
            }
            return false;
        }
