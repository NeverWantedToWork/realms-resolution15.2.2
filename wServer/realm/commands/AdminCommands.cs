...
namespace wServer.realm.commands
{
   ...
   internal class TheHoodTranformation : Command
    ...
    //internal class Omni : Command (This is falcoo, will start command later <3)
    //{
       //public Omni()
           //: base("Omni", 1)
       // {
    ...
    {
        public SetELevel() : base("setelevel", 1) { }
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (string.IsNullOrEmpty(args[0]))
            {
        ...
        {
            {
                ...
                if (((player.ObjectDesc.MaxHitPoints - player.Stats[0]) / 5) * 5 <= ((player.ObjectDesc.MaxHitPoints - player.Stats[0]) / 5))
                {
                    player.SendInfo((player.ObjectDesc.MaxHitPoints - player.Stats[0]) + " (" + (((player.ObjectDesc.MaxHitPoints - player.Stats[0])) / 5) + ") to max health");
                }
                else if (((player.ObjectDesc.MaxHitPoints - player.Stats[0]) / 5) * 5 >= ((player.ObjectDesc.MaxHitPoints - player.Stats[0]) / 5))
                {
                    player.SendInfo((player.ObjectDesc.MaxHitPoints - player.Stats[0]) + " (" + (((player.ObjectDesc.MaxHitPoints - player.Stats[0])) / 5 + 1) + ") to max health");
                }
                if (((player.ObjectDesc.MaxMagicPoints - player.Stats[1]) / 5) * 5 <= ((player.ObjectDesc.MaxMagicPoints - player.Stats[1]) / 5))
                {
                    player.SendInfo((player.ObjectDesc.MaxMagicPoints - player.Stats[1]) + " (" + (((player.ObjectDesc.MaxMagicPoints - player.Stats[1])) / 5) + ") to max mana");
                }
                else if (((player.ObjectDesc.MaxMagicPoints - player.Stats[1]) / 5) * 5 >= ((player.ObjectDesc.MaxMagicPoints - player.Stats[1]) / 5))
                {
                    player.SendInfo((player.ObjectDesc.MaxMagicPoints - player.Stats[1]) + " (" + (((player.ObjectDesc.MaxMagicPoints - player.Stats[1])) / 5 + 1) + ") to max mana");
                }
                player.SendInfo((player.ObjectDesc.MaxAttack - player.Stats[2]) + " (" + (((player.ObjectDesc.MaxAttack - player.Stats[2]))) + ") to max attack");
                player.SendInfo((player.ObjectDesc.MaxDefense - player.Stats[3]) + " (" + (((player.ObjectDesc.MaxDefense - player.Stats[3]))) + ") to max defense");
                player.SendInfo((player.ObjectDesc.MaxSpeed - player.Stats[4]) + " (" + (((player.ObjectDesc.MaxSpeed - player.Stats[4]))) + ") to max speed");
                player.SendInfo((player.ObjectDesc.MaxHpRegen - player.Stats[5]) + " (" + (((player.ObjectDesc.MaxHpRegen - player.Stats[5]))) + ") to max vitality");
                player.SendInfo((player.ObjectDesc.MaxMpRegen - player.Stats[6]) + " (" + (((player.ObjectDesc.MaxMpRegen - player.Stats[6]))) + ") to max wisdom");
                player.SendInfo((player.ObjectDesc.MaxDexterity - player.Stats[7]) + " (" + (((player.ObjectDesc.MaxDexterity - player.Stats[7]))) + ") to max dexterity");
            }
            return true;
        }
    }
