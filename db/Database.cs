        ...
        public Account GetAccount(string id, XmlData data, string uuid=null, string password=null)
        {
            ...
            using (MySqlDataReader rdr = cmd.ExecuteReader())
            {
                ...
                ret = new Account
                {
                    ...
                    EnchantmentLevel = rdr.GetInt32("enchantmentLevel")
                };
            }
            ReadStats(ret);
...
public int UpdateOnraneCurrency(Account acc, int amount)
        {
            MySqlCommand cmd = CreateQuery();
            if (amount > 0)
            {
                cmd.CommandText = "UPDATE stats SET totalOnrane = totalOnrane + @amount WHERE accID=accId;";
                cmd.Parameters.AddWithValue("@accId", acc.AccountId);
                cmd.Parameters.AddWithValue("@amount", amount);
                cmd.ExecuteNonQuery();
                cmd = CreateQuery();
            }
            cmd.CommandText = @"UPDATE stats SET onraneCurrency = onraneCurrency + (@amount) WHERE accId =@accId;
SELECT onraneCurrency FROM stats WHERE accId=@accId;";
            cmd.Parameters.AddWithValue("@accId", acc.AccountId);
            cmd.Parameters.AddWithValue("@amount", amount);
            return (int)cmd.ExecuteScalar();
        }
        ...
        public void ReadStats(Account acc)
        {
            ...
            using (MySqlDataReader rdr = cmd.ExecuteReader())
            {
                if (rdr.HasRows)
                {
                    ...
                    acc.onraneCurrency = rdr.GetInt32("onraneCurrency");
                    acc.Stats = new Stats
                    {
...
VALUES(@accId, @petId, @objType, @skinName, @skin, @rarity, @maxLevel, @abilities, @levels, @xp);";
        ...
        public bool SaveChars(string oldAccId, Chars oldChars, Chars chrs, XmlData data)
        {
            try
            {
                ...
                var cmd = CreateQuery();
                cmd.CommandText = "UPDATE accounts SET prodAcc=1, name=@name, namechosen=@nameChoosen, vaultCount=@vaults, maxCharSlot=@maxChars, ownedSkins=@skins, gifts=@gifts, enchantmentLevel=@enchantmentLevel WHERE id=@oldAccId;";
                ...
                cmd.Parameters.AddWithValue("@enchantmentLevel", chrs.Account.EnchantmentLevel);
                cmd.ExecuteNonQuery();
